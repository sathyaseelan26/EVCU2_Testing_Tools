...
def add_Test_Variable(final_path,port_name,file_name):
    ...
    with open(final_path,"r+") as source_code, open(final_path+".edited","w") as edited_code:
        ...
        for line in source_code:
            ...
            if (((port_name + "_" + "OpRetVal") in line and "Rte_Read_" in line) or (next_line_req == 1)):
                if ('(' in line and ')' in line):
                    ...
                    #print(var_name)
                    global_variable_name = "{}_{}_Opt_ret_Test_{}".format(port_name,file_name[0],count)
                    #print(var_name)
...
wrkng_dir = os.getcwd()
path = wrkng_dir +  r'/BLUEN_EVCU'
# wrkng_dir = r'C:\Users\srr923056\TATA\Stellantis\CAN Testing'
# os.chdir(wrkng_dir)
#print(wrkng_dir)
Port_Data_Type = pd.read_excel('Port_Data_For_E2E.xlsx')
...
os.chdir(wrkng_dir)
df = pd.DataFrame(list(zip(global_variable,global_variable_in_file,port_name_in_excel)),columns = ["Global Variable","File Name","Port"])        
#print(df)
df.to_excel('Global_Variables_declare_Operation_Return.xlsx',engine = 'xlsxwriter',index = False)
# os.chdir(wrkng_dir)