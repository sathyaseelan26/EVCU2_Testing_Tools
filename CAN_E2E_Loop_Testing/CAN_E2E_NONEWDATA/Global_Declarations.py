...
from cantools.database.can import message as Message
...
Output_Test_Cases = r"./Output_Test_Cases"
...
def get_Node_Name(Board: str):
    if Board.upper() == "EVCU":
        ...
        return "MCPB"
    elif Board.upper() == "EDM_SGCP" or Board.upper() == "EDM_MCPC":
        return "SGCP"
    else:
        print("Unknown Board")
        exit()
...
def get_Mapping_Sheets(Mapping_Sheets_Folder):
...
def init_DBCs(DBC_Folder):
    for file in listdir(DBC_Folder):
        if file.endswith(".dbc"):
            if "FD3" in file or "FDCAN3" in file or "CAN_FD3" in file: 
                DBCs["FD3"] = file
            elif "FD5" in file or "FDCAN5" in file or "CAN_FD5" in file:
                DBCs["FD5"] = file
            elif "FD11" in file or "FDCAN11" in file or "CAN_FD11" in file:
                DBCs["FD11"] = file
            elif "FD14" in file or "FDCAN14" in file or "CAN_FD14" in file:
                DBCs["FD14"] = file
            # elif "FD16" in file or "FDCAN16" in file or "CAN_FD16" in file:
            #     DBCs["FD16"] = file       
    pass
