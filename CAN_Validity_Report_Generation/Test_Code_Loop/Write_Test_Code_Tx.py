#Define Test Codes in files (But Global declaration not done) TX
...
def add_Test_Variable(final_path,port_name,file_name,data_type,sig_name,min_val,max_val,inv_val,FD_Ver):
    ...
    with open(final_path,"r+") as source_code, open(final_path+".edited","w") as edited_code:
        ...
        for line in source_code:
            ...
            if (((port_name + "_" + port_name) in line and "Rte_Write_" in line) or (next_line_req == 1)):
                if ('(' in line and ')' in line):
                    #print(line)
                    var_name = line[line.find('(')+1: line.rfind(')')]
                    print(var_name)
                    if ')' in var_name:
                    ...
                    #print(var_name)
                    edited_code.write("\n{} = {};\n".format(var_name,global_variable_name))
...
os.chdir(wrkng_dir)
#print(wrkng_dir)
Port_Data_Type = pd.read_excel('Port_Data_Type_Tx.xlsx')
...
print(df)
df.to_excel('Global_Variables_to_be_declared_Tx.xlsx',engine = 'xlsxwriter',index = False)
os.chdir(wrkng_dir)